...
class Logger {
    constructor(ctx, opts) {
        ...
        for (const k in config.appenders) {
            if (config.appenders[k].type === 'dateFile') {
                config.appenders[k].filename = `/var/log/gpt/${opts.appName}/${config.appenders[k].filename}`;
            }
        }
        configure(config);
...
module.exports = (context,opts) => {
    ...
    return async (ctx, next) => {
        ...
        // 重新赋值,以便取出当次的trace-id(用于后续acc和sql日志)
